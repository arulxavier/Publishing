...
public class PrintUtil {
	...
	public static void createPDFForSubscriber(java.util.List<Subscriber> subscribers, 
		...
		try {
			...
			PrintUtil.addEmptyLine(preface, 1);
	...
	public static void createPDFForSubscriberInfo(
			java.util.List<Subscription> subscriptions, boolean isSingle,
			String fileName) {
		...
		try {
			...
			if (subscriptions != null && !subscriptions.isEmpty()) {
				...
				for (Subscription subscription : subscriptions) {
					...
					map.put(i, subscription);
					i++;
				}
				
				for (int j = 1; j <= map.size(); ) {
					
					Subscription info = map.get(j);
					Subscription info2 = map.get(j+1);
					
					StringBuffer fullAddress = new StringBuffer();
					if (info != null) {
						
						fullAddress.append(info.getSubscriptionCode() + "   \n");
					...
					if (info2 != null) {
						...
						fullAddress1.append(info.getSubscriptionCode() + "   \n");
